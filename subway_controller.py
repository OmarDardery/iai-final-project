...
try:
    ...
    print(f"⚠️ PyAutoGUI failed to load (likely due to Wayland): {e}")
    print("⚠️ Running in MOCK Mode. Key presses will be printed to the terminal instead.")
    class MockPyAutoGUI:
        def press(self, key):
...
# Setup the Pose function for the image
pose_image = mp_pose.Pose(static_image_mode = True, min_detection_confidence=0.5, model_complexity=1)

# Setup the Pose function for the video
...
def detectPose(image, pose, draw=False, display=False):
    ...
    # Otherwise
        ...
        # Return the output image with landmarks
    ...
    if results.pose_landmarks:
        ...
        if game_started:
            ...
            if checkHandsJoined(frame, results)[1] == 'Hands Joined':
                ...
                cv2.putText(frame, f'Counter: {counter}/{num_of_frames}', (10, 70), 
                ...
                if counter == num_of_frames:
                    ...
                    # Move to 1300, 800, then click the left mouse button to start the game and press Space.
